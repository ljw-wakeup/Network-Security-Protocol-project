**Application Layer - email remains to be discussed.**

LI:

[Athanasios Kountouras, Panagiotis Kintis, Chaz Lever, Yizheng Chen, Yacin Nadji, David Dagon, Manos Antonakakis, Rodney Joffe:
Enabling Network Security Through Active DNS Datasets. 188-208](https://doi.org/10.1007/978-3-319-45719-2_9)

[Sajjad Arshad, Amin Kharraz, William Robertson:
Identifying Extension-Based Ad Injection via Fine-Grained Web Content Provenance. 415-436](https://doi.org/10.1007/978-3-319-45719-2_19)

LIN:

[Detecting TCP-Based DDoS Attacks in Baidu Cloud Computing Data Centers](https://ieeexplore.ieee.org/document/8069091)

[Intrusion Detection by Deep Learning with TensorFlow](https://ieeexplore.ieee.org/document/8701969)

HU:

[Ivan Torroledo, Luis David Camacho, Alejandro Correa Bahnsen:
Hunting Malicious TLS Certificates with Deep Neural Networks. 64-73](https://doi.org/10.1145/3270101.3270105)

[Peter Schneider, Konstantin Böttinger:
High-Performance Unsupervised Anomaly Detection for Cyber-Physical System Networks. 1-12](https://doi.org/10.1145/3264888.3264890)

# 3 CSS

## AISec workshop

### 11th AISec@CCS 2018 - AI for Detecting Attacks

[Chao Xu, Zhentan Feng, Yizheng Chen, Minghua Wang, Tao Wei:
FeatNet: Large-scale Fraud Device Detection by Network Representation Learning with Rich Features. 57-63](https://doi.org/10.1145/3270101.3270109)

we propose an effective device fraud detection framework called FeatNet, which incorporates device features and device relationships in network representation learning

[Ivan Torroledo, Luis David Camacho, Alejandro Correa Bahnsen:
Hunting Malicious TLS Certificates with Deep Neural Networks. 64-73](https://doi.org/10.1145/3270101.3270105)

a method for identifying malicious use of web certificates using deep neural networks， on TSL.

### 10th AISec@CCS 2017 - Authentication and Intrusion Detection

[Shuchu Han, Yifan Hu, Steven Skiena, Baris Coskun, Meizhu Liu, Hong Qin, Jaime Perez:
Generating Look-alike Names For Security Challenges. 57-67](https://doi.org/10.1145/3128572.3140441)

behavior-based security challenges, generating fake contacts

[Saumya Solanki, Gautam Krishnan, Varshini Sampath, Jason Polakis:
In (Cyber)Space Bots Can Hear You Speak: Breaking Audio CAPTCHAs Using OTS Speech Recognition. 69-80](https://doi.org/10.1145/3128572.3140443)

audio captcha ecosystem,  off-the-shelf (OTS) speech recognition services can be misused by attackers for trivially bypassing the most popular audio captchas.

[Ram Shankar Siva Kumar, Andrew Wicker, Matt Swann:
Practical Machine Learning for Cloud Intrusion Detection: Challenges and the Way Forward. 81-90](https://doi.org/10.1145/3128572.3140445)

Model evaluation?  described the successful operationalization of machine learning based security detections in a cloud environment (感觉比较相关)

### 9th AISec@CCS 2016

[Paolo Russu, Ambra Demontis, Battista Biggio, Giorgio Fumera, Fabio Roli:
Secure Kernel Machines against Evasion Attacks. 59-69](https://doi.org/10.1145/2996758.2996771)

to develop secure kernel machines against evasion attacks that are not computationally more demanding than their non-secure counterparts
a linear classifier can be drastically improved by selecting a proper regularizer

[Mohsen Ghassemi, Anand D. Sarwate, Rebecca N. Wright:
Differentially Private Online Active Learning with Applications to Anomaly Detection. 117-128](https://doi.org/10.1145/2996758.2996766)

proposes two differentially private strategies to mitigate privacy risk when training a classifier for anomaly detection in an online setting

## 9th MIST@CCS 2017

[Shih-Chieh Su:
Behavior Prediction over Summarized Network Activities. 89-92](https://doi.org/10.1145/3139923.3139934)

(↑ Not that relevent, more likely to be prevention of insider threat)

# 6 RAID (MANY PAPERS RELEVENT)

## 21st RAID 2018 - Intrusion Detection and Prevention

[Hugo Gascon, Steffen Ullrich, Benjamin Stritter, Konrad Rieck:
Reading Between the Lines: Content-Agnostic Detection of Spear-Phishing Emails. 69-91](https://doi.org/10.1007/978-3-030-00470-5_4)

Email spoofing

[Sam L. Thomas, Aurélien Francillon:
Backdoors: Definition, Deniability and Detection. 92-113](https://doi.org/10.1007/978-3-030-00470-5_5)

as its name.

[Shagufta Mehnaz, Anand Mudgerikar, Elisa Bertino:
RWGuard: A Real-Time Detection System Against Cryptographic Ransomware. 114-136](https://doi.org/10.1007/978-3-030-00470-5_6)

ransomware detection mechanism, RWGuard

## 20th RAID 2017 - Intrusion Detection

[Ahmet Salih Buyukkayhan, Alina Oprea, Zhou Li, William K. Robertson:
Lens on the Endpoint: Hunting for Malicious Software Through Endpoint Data Analysis. 73-97](https://doi.org/10.1007/978-3-319-66332-6_4)

Enterprise malware detection

[Amin Kharraz, Engin Kirda:
Redemption: Real-Time Protection Against Ransomware at End-Hosts. 98-119](https://doi.org/10.1007/978-3-319-66332-6_5)

[Anaël Beaugnon, Pierre Chifflier, Francis Bach:
ILAB: An Interactive Labelling Strategy for Intrusion Detection. 120-140](https://doi.org/10.1007/978-3-319-66332-6_6)

a novel interactive labelling strategy that helps experts label large datasets for intrusion detection with a reduced workload

## 19th RAID 2016

Low-Level Attacks and Defenses

[Tianwei Zhang, Yinqian Zhang, Ruby B. Lee:
CloudRadar: A Real-Time Side-Channel Attack Detection System in Clouds. 118-140](https://doi.org/10.1007/978-3-319-45719-2_6)

defend cache-based side-channel attacks in multi-tenant cloud systems.

Measurement Studies

[Athanasios Kountouras, Panagiotis Kintis, Chaz Lever, Yizheng Chen, Yacin Nadji, David Dagon, Manos Antonakakis, Rodney Joffe:
Enabling Network Security Through Active DNS Datasets. 188-208](https://doi.org/10.1007/978-3-319-45719-2_9)

a system, Thales, that actively queries and collects records for massive amounts of domain names from various seeds

(↑ Databases are one direction)

Web and Mobile Security

[Giancarlo Pellegrino, Onur Catakoglu, Davide Balzarotti, Christian Rossow:
Uses and Abuses of Server-Side Requests. 393-414](https://doi.org/10.1007/978-3-319-45719-2_18)

n automated scanner we developed to probe web applications to identify possible SSR misuses

[Sajjad Arshad, Amin Kharraz, William Robertson:
Identifying Extension-Based Ad Injection via Fine-Grained Web Content Provenance. 415-436](https://doi.org/10.1007/978-3-319-45719-2_19)

web extensions

present a fine-grained approach to tracking the provenance of web content at the level of individual DOM elements

# 4 S&P

Extending Detection with Privileged Information via Generalized Distillation
神经网络 与 特权攻击检测
https://ieeexplore.ieee.org/document/8424637

Detecting Homoglyph Attacks with a Siamese Neural Network
同名攻击与神经网络
https://ieeexplore.ieee.org/document/8424628

Machine Learning DDoS Detection for Consumer Internet of Things Devices
物联网攻击与机器学习检测
https://ieeexplore.ieee.org/document/8424629

A Deep Learning Approach to Fast, Format-Agnostic Detection of Malicious Web Content
https://ieeexplore.ieee.org/document/8424626）
深度学习与内容安全检测

# 1 TDSC

Low-Rate DoS Attacks Detection Based on Network Multifractal
检测低速拒绝服务攻击，小波分析等等
https://ieeexplore.ieee.org/document/7120917

Distributed Attack Graph Generation
分布攻击图的生成
https://ieeexplore.ieee.org/document/7087377

Industrial Control System Network Intrusion Detection by Telemetry Analysis
工业控制系统入侵的检测
https://ieeexplore.ieee.org/document/7122336

A Support Vector Machine-Based Framework for Detection of Covert Timing Channel
知识向量机与隐蔽信道
https://ieeexplore.ieee.org/document/7087364

Leveraging Strategic Detection Techniques for Smart Home Pricing Cyberattacks
智能家居定价网络攻击 马尔可夫
https://ieeexplore.ieee.org/document/7115920

A System Architecture for the Detection of Insider Attacks in Big Data Systems
大数据与内部攻击
https://ieeexplore.ieee.org/document/8093643

Learning from the Ones that Got Away: Detecting New Forms of Phishing Attacks（钓鱼攻击与机器学习https://ieeexplore.ieee.org/document/8440723）

CSC-Detector: A System to Infer Large-Scale Probing Campaigns :比较迷幻 是检查暗网数据的https://ieeexplore.ieee.org/document/7516606

Risk Assessment in Social Networks Based on User Anomalous Behaviors: In this paper, we make a step towards the definition of a unique tool that helps OSN providers as well as users to detect several types of attacks and, therefore, to have a global understanding of risky users in OSNs.(https://ieeexplore.ieee.org/document/7430280)

Exact Inference Techniques for the Analysis of Bayesian Attack Graphs(https://ieeexplore.ieee.org/document/7885532)

Defending Against Web Application Attacks: Approaches, Challenges and Implications

G-SIR: An Insider Attack Resilient Geo-Social Access Control Framework
解决内部攻击 In this paper, we propose a Geo-Social Insider Threat Resilient Access Control Framework (G-SIR) to deter insider threats by including current and historic geo-social information as part of the access control decision process.
https://ieeexplore.ieee.org/document/7820131

# 2 TIFS

[1.Node State Monitoring Scheme in Fog Radio Access Networks for Intrusion Detection](https://ieeexplore.ieee.org/document/8640037)

[2.A New Architecture for Network Intrusion Detection and Prevention](https://ieeexplore.ieee.org/document/8630944)

[3.A Survey of Network-based Intrusion Detection Data Sets](https://arxiv.org/abs/1903.02460)

[4.Real-Time Detection of Hybrid and Stealthy Cyber-Attacks in Smart Grid](https://ieeexplore.ieee.org/document/8409487)

[5.Distributed Quickest Detection of Cyber-Attacks in Smart Grid](https://ieeexplore.ieee.org/document/8278264)



# 5 USENIX Security

[1.End-to-End Measurements of Email Spoofing Attacks](https://www.usenix.org/conference/usenixsecurity18/presentation/hu)

[2.WPSE: Fortifying Web Protocols via Browser-Side Security Monitoring](https://www.usenix.org/conference/usenixsecurity18/presentation/calzavara)

[3.6thSense: A Context-aware Sensor-based Attack Detector for Smart Devices](https://www.usenix.org/conference/usenixsecurity17/technical-sessions/presentation/sikder)

[4.White-Stingray: Evaluating IMSI Catchers Detection Applications](https://www.usenix.org/conference/woot17/workshop-program/presentation/park)
